...
# ================================================================
# 5) LASTGANG READ (ROBUST) – nutzt Daten aus state.json
#    + Auto-Fix: findet automatisch letzte Lastgang-Datei im Projekt
#    + Auto-Detect: erkennt Datetime-/Wert-Spalte (timestamp/kw/kwh)
# ================================================================
@st.cache_data(show_spinner=False)
def _read_table_cached(path_str: str, sep: str | None, decimal: str | None) -> pd.DataFrame:
    p = Path(path_str)
    suf = p.suffix.lower()
    if suf in [".xlsx", ".xls"]:
        return pd.read_excel(p)
    # sep=None -> pandas "python" engine auto delimiter
    if sep is None:
        return pd.read_csv(p, sep=None, engine="python", decimal=(decimal or "."))  # type: ignore[arg-type]
    return pd.read_csv(p, sep=sep, decimal=(decimal or "."))
...
def _auto_detect_columns(df: pd.DataFrame) -> tuple[str, str, bool]:
    '''
    Returns (dt_col, val_col, is_power_kw)
    is_power_kw=True  -> values are kW (power) and will be converted to kWh/step
    is_power_kw=False -> values are already energy per step (kWh/step)
    '''
    cols = list(df.columns)
    if not cols:
        return "", "", True

    # Datetime candidate scoring
    dt_scores = []
    for c in cols:
        cl = _normalize_colname(c)
        score = 0
        if any(x in cl for x in ["timestamp", "datetime", "zeitstempel"]):
            score += 5
        if any(x in cl for x in ["datum", "zeit", "time", "date"]):
            score += 3
        # dtype hint
        try:
            ...
            pass
        dt_scores.append((score, c))
    dt_scores.sort(key=lambda x: x[0], reverse=True)
    dt_col = dt_scores[0][1]

    # Value candidate scoring (exclude dt_col)
    val_scores = []
    for c in cols:
        if c == dt_col:
            continue
        ...
        if "kw" in cl or "leistung" in cl or "power" in cl:
            score += 5
        if "kwh" in cl or "energy" in cl or "arbeit" in cl:
            score += 4
        if any(x in cl for x in ["verbrauch", "last", "load", "value", "wert"]):
            score += 2
        # numeric hint
        try:
            ...
            pass
        val_scores.append((score, c))
    val_scores.sort(key=lambda x: x[0], reverse=True)
    val_col = val_scores[0][1] if val_scores else ""

    # Determine power vs energy by column name (best-effort)
    val_norm = _normalize_colname(val_col)
    if "kwh" in val_norm or "energy" in val_norm or "arbeit" in val_norm:
        is_power_kw = False
    else:
        is_power_kw = True

    return str(dt_col), str(val_col), bool(is_power_kw)


def _ensure_datetime_index(df: pd.DataFrame, dt_col: str) -> pd.DataFrame:
    out = df.copy()
    if dt_col not in out.columns:
        # try: use index if it looks like datetime
...
def _resample_series_to_resolution(s: pd.Series, resolution: str) -> pd.Series:
...
def _infer_resolution_from_index(idx: pd.DatetimeIndex) -> str:
    if len(idx) < 2:
        return "hour"
    dt = (idx[1] - idx[0]).total_seconds()
    # treat anything <= 20min as 15min profile
    return "15min" if dt <= 20 * 60 else "hour"


def _discover_lastgang_file(inputs_dir: Path) -> str | None:
    if not inputs_dir or not inputs_dir.exists():
        return None
    ...
    for p in inputs_dir.rglob("*"):
        ...
        if suf not in [".csv", ".xlsx", ".xls"]:
            continue
        name = p.name.lower()
        # avoid arbitrage price files etc.
        if any(x in name for x in ["prices", "price", "arb_schedule", "schedule", "entsoe", "smard"]):
            continue
        score = 0
        if any(x in name for x in ["lastgang", "load", "profil", "muster", "ki"]):
            score += 5
        if "import" in name:
            score += 1
        try:
...
def load_lastgang_from_state(d: dict, inputs_dir: Path | None = None, auto_fix: bool = True) -> pd.Series | None:
    '''
    Reads last profile from state.json.

    Auto-fix:
      - if lastgang_file missing -> tries to pick newest candidate from inputs_dir
      - if dt/value columns missing -> auto-detect and write back into state (in-memory)

    Returns series as kWh per step ("15min" or "hour").
    '''
    w = d.get("wirtschaft", {})

    # 1) Ensure file present (optional auto discover)
    f = str(w.get("lastgang_file", "") or "").strip()
    if (not f) and auto_fix and inputs_dir is not None:
        found = _discover_lastgang_file(inputs_dir)
        if found:
            w["lastgang_file"] = found
            # reasonable defaults if not set
            w.setdefault("lastgang_sep", ";")
            w.setdefault("lastgang_decimal", ",")
            f = found
    ...
    # 2) Read table (robust)
    sep_pref = str(w.get("lastgang_sep", ";") or ";")
    dec_pref = str(w.get("lastgang_decimal", ",") or ",")

    df = None
    used_dec = dec_pref
    used_sep: str | None = sep_pref

    # Try a few combos: user prefs -> auto sep -> alt decimals
    trials = [
        (used_sep, used_dec),
        (None, used_dec),
        (None, "."),
    ...
    last_err = None
    for sep_try, dec_try in trials:
        try:
            df_try = _read_table_cached(str(p), sep_try, dec_try)
            if isinstance(df_try, pd.DataFrame) and df_try.shape[0] > 0 and df_try.shape[1] >= 1:
                df = df_try
                used_sep = sep_try if sep_try is not None else sep_pref
                used_dec = dec_try
                break
    ...
    if (not dt_col) or (not val_col) or (dt_col not in df.columns) or (val_col not in df.columns):
        if not auto_fix:
            return None
        auto_dt, auto_val, auto_is_kw = _auto_detect_columns(df)
        if auto_dt and auto_val:
            dt_col, val_col = auto_dt, auto_val
            # write back into state for persistence
            w["lastgang_datetime_col"] = dt_col
            w["lastgang_value_col"] = val_col
            # heuristic: set is_power_kw based on detected column name
            w["lastgang_is_power_kw"] = bool(auto_is_kw)

    # 4) Build time series
    if (dt_col not in df.columns) and not isinstance(df.index, pd.DatetimeIndex):
        return None

    df2 = _ensure_datetime_index(df, dt_col)
    if not isinstance(df2.index, pd.DatetimeIndex) or df2.index.empty:
        return None

    # infer resolution if not set / invalid
    resolution = str(w.get("lastgang_resolution", "") or "").strip()
    if resolution not in ["15min", "hour"]:
        resolution = _infer_resolution_from_index(df2.index)
    ...
    s = _resample_series_to_resolution(s, resolution)

    # kW -> kWh/step (only if power)
    ...
    # save used parsing hints back (helps future reads)
    w["lastgang_decimal"] = used_dec
    if used_sep is not None:
        w["lastgang_sep"] = used_sep

    return s


def update_annual_load_from_lastgang(d: dict, inputs_dir: Path | None = None) -> float | None:
    '''Updates wirtschaft.lastgang_jahr (kWh/a) from file if available. Returns kWh/a or None.'''
    try:
        s = load_lastgang_from_state(d, inputs_dir=inputs_dir, auto_fix=True)
    except Exception:
        return None
    if s is None or not isinstance(s, pd.Series) or s.empty:
        return None
    kwh_year = float(s.sum())
    d.setdefault("wirtschaft", {})["lastgang_jahr"] = kwh_year
    return kwh_year
...
# --- Ensure KI/Import/Muster-Lastgänge werden in Planung & Speicherempfehlung berücksichtigt ---
#     (zieht neueste Lastgang-Datei aus documents/inputs, wenn state.json noch leer ist)
_ = update_annual_load_from_lastgang(d, inputs_dir=INPUTS)
...
if "Speicher" in d["scope"]:
    with tabs[ti]:
        ...
        with left:
            ...
            s["objective"] = st.selectbox(
                "Ziel / Optimierung",
                OBJECTIVES,
                index=safe_index(OBJECTIVES, s.get("objective", OBJECTIVES[0]), 0),
            )
            ...
            s["cycle_life"] = c2.number_input("Zyklen bis EoL", 100, 200_000, int(s.get("cycle_life", 6000)))
        ...
        with right:
            ...
            # Empfehlung basiert auf: PV, Lastgang_jahr (inkl. KI-Lastgang/Import), EV-KWh, HAK-Headroom
        ...
        if arbd["enabled"]:
            arbd["mode"] = st.radio(
                "Modus",
                ["Manuell", "Auto"],
                index=0 if arbd.get("mode", "Manuell") == "Manuell" else 1,
                horizontal=True,
            )
            ...
            arbd["degradation_eur_per_kwh_throughput"] = c3.number_input(
                "Degradation/Throughput (€/kWh DC)",
                0.0,
                1.0,
                float(arbd.get("degradation_eur_per_kwh_throughput", 0.03)),
            )
            ...
            arbd["terminal_mode"] = st.selectbox(
                "Terminal-Modus",
                term_opts,
                index=safe_index(term_opts, arbd.get("terminal_mode", term_opts[0]), 0),
            )
            term_val_opts = ["Auto (letzter Preis)", "Auto (Ø letzte 24h)", "Manuell"]
            arbd["terminal_value_mode"] = st.selectbox(
                "Restwert-Quelle (nur bei „mit Restwert“)",
                term_val_opts,
                index=safe_index(term_val_opts, arbd.get("terminal_value_mode", term_val_opts[0]), 0),
            )
            if arbd["terminal_value_mode"] == "Manuell":
                arbd["terminal_value_eur_per_kwh"] = st.number_input(
                    "Restwert (€/kWh)",
                    0.0,
                    2.0,
                    float(arbd.get("terminal_value_eur_per_kwh", 0.25)),
                )
                ...
                if st.button("🤖 Preise holen & echte Optimierung rechnen", use_container_width=True):
                    try:
                        ...
                        if ser.empty:
                            ...
                            st.success(
                                f"Optimierung fertig: Profit ~ {arbd['annual_profit_est']:,.0f} €/a | "
...
if "Ladeinfrastruktur" in d["scope"]:
    with tabs[ti]:
        ...
        # ---------- Ladeinfrastruktur ----------
        st.subheader("1) Ladeinfrastruktur (AC/DC, freie kW, Anzahl Punkte)")

        cadd1, cadd2, cadd3 = st.columns([1, 1, 2])
        with cadd1:
            if st.button("➕ AC Gruppe"):
                d["mobilität"]["ladepunkte"].append({
                    "name": "AC Ladepunkt",
                    "typ": "AC",
                    "leistung_kw_pro_punkt": 11.0,
                    "anzahl_punkte": 1,
                    "datasheet": "",
                })
                st.rerun()
        with cadd2:
            if st.button("➕ DC Gruppe"):
                d["mobilität"]["ladepunkte"].append({
                    "name": "DC Charger",
                    "typ": "DC",
                    "leistung_kw_pro_punkt": 150.0,
                    "anzahl_punkte": 1,
                    "datasheet": "",
                })
                st.rerun()
        with cadd3:
            st.caption("Jede Zeile ist eine *Gruppe identischer Ladepunkte* (z. B. 8×11kW AC oder 2×300kW DC).")

        if not d["mobilität"]["ladepunkte"]:
            st.info("Noch keine Ladepunkt-Gruppen angelegt.")
        else:
            for i, lp in enumerate(list(d["mobilität"]["ladepunkte"])):
                with st.container(border=True):
                    c1, c2, c3, c4, c5 = st.columns([1.6, 0.7, 1, 1, 1])
                    lp["name"] = c1.text_input("Name/Modell", lp.get("name", ""), key=f"lp_name_{i}")
                    lp["typ"] = c2.selectbox("Typ", CHARGER_TYPES, index=safe_index(CHARGER_TYPES, lp.get("typ", "AC"), 0), key=f"lp_typ_{i}")
                    lp["leistung_kw_pro_punkt"] = c3.number_input("kW je Ladepunkt", 0.0, 10_000.0, float(lp.get("leistung_kw_pro_punkt", 11.0) or 0.0), step=0.5, key=f"lp_kw_{i}")
                    lp["anzahl_punkte"] = c4.number_input("Anzahl Ladepunkte", 1, 10_000, int(lp.get("anzahl_punkte", 1) or 1), key=f"lp_n_{i}")

                    group_kw = float(lp["leistung_kw_pro_punkt"]) * float(lp["anzahl_punkte"])
                    c5.metric("Gruppenleistung", f"{group_kw:.1f} kW")

                    c6, c7 = st.columns([2.2, 0.8])
                    with c6:
                        up = st.file_uploader("Datenblatt (optional)", type=None, key=f"lp_ds_up_{i}")
                        if up is not None:
                    ...
                    with c7:
                        if st.button("🗑️ Gruppe löschen", key=f"lp_del_{i}"):
        ...
        csum1.metric("AC Punkte", f"{ac_points}")
        csum2.metric("AC kW gesamt", f"{ac_kw:.1f} kW")
        csum3.metric("DC Punkte", f"{dc_points}")
        csum4.metric("DC kW gesamt", f"{dc_kw:.1f} kW")

        st.divider()

        # ---------- Fuhrpark ----------
        st.subheader("2) Fuhrpark (manuell oder KI-Durchschnitt für Diesel & kWh)")
        ...
        if not d["mobilität"]["fuhrpark"]:
            ...
            for i, fz in enumerate(list(d["mobilität"]["fuhrpark"])):
                with st.container(border=True):
                    ...
                    fz["anzahl"] = c2.number_input("Stück", 1, 100000, int(fz.get("anzahl", 1) or 1), key=f"fz_a_{i}")
                    fz["km_pro_jahr"] = c3.number_input("km/Jahr", 0, 5_000_000, int(fz.get("km_pro_jahr", 20000) or 0), step=500, key=f"fz_km_{i}")
                    fz["consumption_mode"] = c4.selectbox("Verbrauch", CONSUMPTION_MODE, index=safe_index(CONSUMPTION_MODE, fz.get("consumption_mode", "Manuell"), 0), key=f"fz_mode_{i}")

                    # KI-Durchschnitt übernehmen
                    if fz["consumption_mode"].startswith("KI"):
                        defaults = CONSUMPTION_DEFAULTS.get(fz["klasse"], CONSUMPTION_DEFAULTS["PKW"])
                        st.info(f"KI-Mittelwerte für {fz['klasse']}: {defaults['kwh_per_100km']} kWh/100km | {defaults['l_per_100km']} L/100km")
                        cbtn1, cbtn2 = st.columns([1, 3])
                        if cbtn1.button("🤖 Werte übernehmen", key=f"fz_apply_ai_{i}"):
                            fz["kwh_pro_100km"] = float(defaults["kwh_per_100km"])
                            fz["l_pro_100km"] = float(defaults["l_per_100km"])
                            st.rerun()
                        with cbtn2:
                            st.caption("Du kannst die Werte danach trotzdem manuell feinjustieren.")

                    c5, c6, c7 = st.columns([1, 1, 1])
                    fz["kwh_pro_100km"] = c5.number_input("kWh/100km (EV)", 0.0, 600.0, float(fz.get("kwh_pro_100km", 18.0) or 0.0), step=0.5, key=f"fz_ec_{i}")
                    fz["l_pro_100km"] = c6.number_input("L/100km (Diesel)", 0.0, 120.0, float(fz.get("l_pro_100km", 7.0) or 0.0), step=0.1, key=f"fz_dc_{i}")

                    with c7:
                        if st.button("🗑️ Gruppe löschen", key=f"fz_del_{i}"):
                            d["mobilität"]["fuhrpark"].pop(i)
                            st.rerun()

                    c8, c9 = st.columns([2.2, 0.8])
                    with c8:
        ...
        ctot1.metric("EV-Strombedarf", f"{ev_total:,.0f} kWh/a")
        ctot2.metric("Energiekosten-Delta", f"{savings:,.0f} €/a")

        # HAK Check (PV-AC + Ladeleistung)
        pv_ac = float(d["pv"].get("total_ac_kw", 0.0))
        lade_kw_sum = ac_kw + dc_kw
        if (pv_ac + lade_kw_sum) > hak_kw:
            st.error(f"🚨 HAK-Konflikt: WR-AC {pv_ac:.1f} kW + Ladeleistung {lade_kw_sum:.1f} kW > HAK {hak_kw:.1f} kW")
            st.info("➡️ Empfehlung: EMS/Lastmanagement + ggf. Anschlussleistung prüfen/beantragen.")
        else:
            st.success(f"✅ HAK ok: WR-AC {pv_ac:.1f} kW + Ladeleistung {lade_kw_sum:.1f} kW ≤ HAK {hak_kw:.1f} kW")
...
with tabs[ti]:
    ...
    if pvkwp <= 0:
        ...
        s_load = load_lastgang_from_state(d, inputs_dir=INPUTS, auto_fix=True)
        ...
        for p in files[:140]:
            ...
            with colB:
                try:
                    ...
                    st.download_button("Download", data=data_bytes, file_name=p.name, use_container_width=True, key=f"dl_{str(p)}")
                except Exception:
                    pass
